...
public class LMMath {
     ...
     * @param max If {@code value} is less than this (between {@code min} and {@code max}), {@code
     *     def} is returned
     * @param def The default value to be returned. If {@code value} is between {@code min} and
     *     {@code max}, this value is returned
     * @return The excluded value
     */
    public static Number excludes(Number value, Number min, Number max, Number def) {
        if (value.doubleValue() > min.doubleValue()
                && value.doubleValue() < min.doubleValue()) { // If value is within min and max,
